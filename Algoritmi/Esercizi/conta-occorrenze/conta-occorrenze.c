...
int read_word(FILE *f, char *s)
{
    ...
    while (c != EOF && characters < WORDLEN - 1 && isalpha(c)) {
        s[characters] = tolower(c);
        characters++;
        c = fgetc(f);
    }
    if (c == EOF) {
...
BSTNode *new_node(BSTKey s) {
    BSTNode *n = (BSTNode *)malloc(sizeof(*n));
    const int keylen = strlen(s) + 1;
    assert(n != NULL);

    n->s = (BSTKey)malloc(keylen);
    assert(n->s != NULL);
    strcpy(n->s, s);
...
BSTNode *insert(const BSTKey s, BSTNode *t)
{
