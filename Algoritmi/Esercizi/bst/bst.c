...
static BSTNode *bst_new_node(BSTNode *p, BSTKey k) {
...
static int bst_node_is_left_child(const BSTNode *n) {
    return n->parent->left == n;
}

static int bst_has_left_child(const BSTNode *n) {
    return n->left != NULL;
}

static int bst_has_right_child(const BSTNode *n) {
    return n->right != NULL;
}

static int bst_is_leaf(const BSTNode *n) {
    return n->right == NULL && n->left == NULL;
}

static void bst_delete_rec(BST *T, BSTNode *n) {
    BSTNode *x;
    ...
    if (!bst_has_left_child(n) && bst_has_right_child(n)) {
        x = n->right;
    } else if (bst_has_left_child(n) && !bst_has_right_child(n)) {
        x = n->left;
    }
...
static int bst_height_rec(const BSTNode *n)
{
    ...
    height_left = bst_height_rec(n->left);
