...
int ht_insert(HashTable *h, const char *key, int value)
{
    ...
    /* Ottengo l'indice passando il numero di encode alla hash function */
    k = encode(key);
    index = hash_function(h, k);
    found = search_key(h->items[index], &node, key);

    /* Creo un nuovo nodo se non è stato trovato nell'hash table */
    if (!found) {
        h->items[index] = hashtable_new_node(key, value, h->items[index]);
        h->values_count++;
    } else {
        /* Aggiorna il valore nella posizione del nodo in posizione della chiave */
        node->value = value;
        strcpy(node->key, key);
    }
...
HashNode *ht_search(HashTable *h, const char *key)
{
    ...
    /* Se trova la chiave ritorna il nodo contenente la chiave, NULL altrimenti */
    search_key(h->items[index], &node, key);
...
{
    int found;
    HashNode *temp, *prev = NULL, *node = NULL;
    unsigned long k, index;
    ...
    /* Se trova la chiave è presente viene rimossa e ritorna 1, 0 altrimenti */
    found = search_key(h->items[index], &node, key);
    if (found) {
        for (temp = h->items[index]; temp != NULL && !keys_equal(temp->key, key); temp = temp->next) {
            prev = temp;
        }
        if (prev == NULL) {
