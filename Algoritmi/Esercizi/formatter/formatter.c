...
produrrà il seguente risultato (vengono mostrate solo le prime e le
ultime cinque righe; si noti che la prima riga non compare nel file di
input e viene stampata solo come riferimento per verificare
visivamente la lunghezza delle righe):
...
int format_word(const char *w, int line_length, int Lmax) {
    int wlen = strlen(w);

    /* Se la parola è più lunga di Lmax chiude il programma */
    assert(wlen <= Lmax);
    line_length += wlen + 1; /* +1 per includere lo spazio */
    if (line_length > Lmax) {
        line_length = wlen;
        printf("\n");
    }
...
int main( int argc, char *argv[] )
{
    ...
    while ( read_word(filein, w) > 0 ) {
        line_length = format_word(w, line_length, Lmax);
    }
    printf("\n");
