...
float onde(float x, float amplitude) {
	float n = sin(x*8.0 + iTime) * 0.05 +
		sin(x*27.3 + iTime*0.5) * 0.005 + 
		sin(iTime) * 0.01;
	return n * amplitude;
}
...
//Mappiamo le coordinate del frammento nel sistema NDC (sistema di Cordinate Normalizzate di Device)
     ...
     vec2 normalizedCoords = 2.0 * gl_FragCoord.xy / iResolution.xy - 1.0;
      ...
      else
      {
        ...
        float cloudDensity = 0.8;  // Densità delle nuvole
         ...
         for (int i = 0; i < numberOfClouds; i++)
         {
             ...
             vec2 circleCenter = vec2(random(vec2(float(i), iTime * 0.001)) * 2.0 - 1.0,
                 random(vec2(iTime * 0.0001, float(i))) * 2.0 - 1.0);
