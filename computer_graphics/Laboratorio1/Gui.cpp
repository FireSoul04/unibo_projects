...
void my_interface()
{
    ...
    ImGui::SetNextWindowPos(ImVec2(1, 650)); // Imposta posizione finestra successiva (50, 50)
    ...
    */
    ...
    ImGui::Text("Coordinate Mouse : (%.1f, %.1f)", mousex, mousey); // Mostra coordinate mouse
    ImGui::InputFloat("Input float", &my_float); // Input testo per float
    ImGui::InputInt("Input int", &my_int);       // Input testo per int
    ImGui::End(); // Chiude la finestra "Impostazioni"

    // Imposta la posizione iniziale per la finestra successiva "Impostazioni 2" solo alla prima creazione
    ImGui::SetNextWindowPos(ImVec2(1, 50), ImGuiCond_FirstUseEver);

    /*
    ImGuiCond_FirstUseEver fa sì che la posizione sia applicata solo la prima volta che la
    finestra è creata o quando non ci sono dati salvati. Permette quindi all’utente di
    spostare la finestra liberamente dopo l’apertura iniziale.
    */

    ImGui::Begin("Impostazioni 2", NULL, ImGuiWindowFlags_AlwaysAutoResize); // Finestra con auto-resize   
    ImGui::SliderFloat("position x", &positionx, -50.0f, 50.0f);             // Slider float per positionx
    if (ImGui::Button("Click Me")) {                                        // Bottone con azione vuota
        // azione da inserire
    }
    ImGui::SliderFloat("position y", &positiony, -50.0f, 50.0f);            // Slider float per positiony
    ImGui::Separator();                                                      // Linea divisoria orizzontale
    ImGui::Checkbox("Visualizza1", &flag1);                                 // Checkbox per flag1
    ImGui::SameLine();                                                       // Prossimo widget sulla stessa linea
    ImGui::Checkbox("Visualizza3 ", &flag2);                                // Checkbox per flag2 affiancata
    ImGui::End();                                                           // Chiude la finestra "Impostazioni 2"
