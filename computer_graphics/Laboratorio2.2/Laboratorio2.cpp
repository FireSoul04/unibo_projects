...
constexpr unsigned int verticesSize = 3;

float vertices[verticesSize * 3] = { 0 };
float colors[verticesSize * 4] = { 0 };
...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 0; i < verticesSize; i++) {
            vertices[i * 3 + 0] = guiVertices[i].x;
            vertices[i * 3 + 1] = guiVertices[i].y;
            vertices[i * 3 + 2] = 0.0f;

            colors[i * 4 + 0] = guiColors[i].x;
            colors[i * 4 + 1] = guiColors[i].y;
            colors[i * 4 + 2] = guiColors[i].z;
            colors[i * 4 + 3] = guiColors[i].w;
        }
